import { Message, MessageTypeEnum } from "@/lib/types/conversation.type.ts";
    ...
    <>
      {Object.keys(characterDetails).map((key: string) => (
        <div className="flex flex-row gap-2 justify-between w-full" key={key}>
          <h1 className="font-bold capitalize text-white">{key}</h1>
          <div className="flex text-white">
            {editKey === key ? (
              <textarea
                value={editValue}
                className="border-0 h-auto focus-visible:ring-0 focus-visible:outline-none text-black"
